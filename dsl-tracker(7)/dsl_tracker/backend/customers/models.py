...
class Customer(models.Model):
    ...
    user_id = models.CharField(
        ...
        help_text="ISP subscriber/user ID, e.g. lah9201640 — must be unique across all customers",
    )
    ...
    def save(self, *args, **kwargs):
        # Normalize blank -> None so the unique constraint only rejects real
        # duplicate IDs, not multiple customers who simply don't have one yet
        # (a unique CharField would otherwise treat repeated '' as a clash).
        self.user_id = self.user_id or None
...
class DSLService(models.Model):
    ...
    msag_port = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(0), MaxValueValidator(63)],
        help_text="Port number on the card, 0-63 (a DSL line card has 64 ports)",
    )

    ip_address = models.GenericIPAddressField(null=True, blank=True)
    modem_serial = models.CharField(max_length=100, blank=True, null=True, unique=True)
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["msag", "msag_card"]),
        ]
        constraints = [
            # A given port on a given card, within a given MSAG cabinet, can
            # only ever be assigned to one DSL service at a time — this is
            # what makes "port already in use" a clean validation error
            # instead of two customers silently sharing a physical port.
            models.UniqueConstraint(
                fields=["msag", "msag_card", "msag_port"], name="unique_port_per_card"
            ),
        ]
    ...
    def save(self, *args, **kwargs):
        # Same blank -> None normalization as Customer.user_id, and for the
        # same reason: modem_serial is unique, but not every service has a
        # modem serial recorded yet.
        self.modem_serial = self.modem_serial or None
        super().save(*args, **kwargs)
