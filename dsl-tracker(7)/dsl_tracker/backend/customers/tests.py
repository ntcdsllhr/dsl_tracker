...
class CustomerCRUDTests(BaseAPITestCase):
    ...
    def test_retrieve_customer_detail_includes_nested_dsl_and_pair(self):
        ...
        dsl = DSLService.objects.create(
            ...
            msag=self.msag, msag_card="C04", msag_port=12,
        )
...
class SearchAndFilterTests(BaseAPITestCase):
    def setUp(self):
        ...
        self.dsl1 = DSLService.objects.create(
            ...
            msag=self.msag, msag_card="C04", msag_port=12, status=DSLService.Status.ACTIVE,
        )
        self.dsl2 = DSLService.objects.create(
            customer=self.c2, dsl_package="Broadband 4Mbps Unlimited",
            msag=self.msag, msag_card="C02", msag_port=5, status=DSLService.Status.FAULTY,
        )
...
class ActivityLoggingTests(BaseAPITestCase):
    ...
    def test_customer_history_endpoint_returns_all_related_entity_events(self):
        ...
        dsl_resp = self.client.post("/api/dsl-services/", {
            ...
            "msag": self.msag.id, "msag_card": "C04", "msag_port": 12,
        })
...
class BusinessRuleTests(BaseAPITestCase):
    """Data-integrity rules: unique user IDs, unique modem serials, unique
    port-per-card, and the 0-63 port range for a 64-port line card."""

    def test_duplicate_user_id_rejected(self):
        self.client.post("/api/customers/", {
            "name": "A", "address_line": "x", "city": self.city.id,
            "phone_primary": "111", "user_id": "lah9999999",
        })
        resp = self.client.post("/api/customers/", {
            "name": "B", "address_line": "y", "city": self.city.id,
            "phone_primary": "222", "user_id": "lah9999999",
        })
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("user_id", resp.data)

    def test_multiple_customers_without_user_id_are_allowed(self):
        r1 = self.client.post("/api/customers/", {
            "name": "A", "address_line": "x", "city": self.city.id, "phone_primary": "111",
        })
        r2 = self.client.post("/api/customers/", {
            "name": "B", "address_line": "y", "city": self.city.id, "phone_primary": "222",
        })
        self.assertEqual(r1.status_code, status.HTTP_201_CREATED)
        self.assertEqual(r2.status_code, status.HTTP_201_CREATED)

    def test_duplicate_modem_serial_rejected(self):
        c1 = self.make_customer()
        c2 = self.make_customer(name="Other", phone_primary="999", user_id="u999")
        self.client.post("/api/dsl-services/", {
            "customer": c1.id, "dsl_package": "X", "msag": self.msag.id,
            "msag_card": "C01", "msag_port": 1, "modem_serial": "SN-0001",
        })
        resp = self.client.post("/api/dsl-services/", {
            "customer": c2.id, "dsl_package": "X", "msag": self.msag.id,
            "msag_card": "C02", "msag_port": 2, "modem_serial": "SN-0001",
        })
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("modem_serial", resp.data)

    def test_multiple_services_without_modem_serial_are_allowed(self):
        c1 = self.make_customer()
        c2 = self.make_customer(name="Other", phone_primary="999", user_id="u999")
        r1 = self.client.post("/api/dsl-services/", {
            "customer": c1.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C01", "msag_port": 1,
        })
        r2 = self.client.post("/api/dsl-services/", {
            "customer": c2.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C02", "msag_port": 2,
        })
        self.assertEqual(r1.status_code, status.HTTP_201_CREATED)
        self.assertEqual(r2.status_code, status.HTTP_201_CREATED)

    def test_duplicate_port_on_same_card_rejected(self):
        c1 = self.make_customer()
        c2 = self.make_customer(name="Other", phone_primary="999", user_id="u999")
        self.client.post("/api/dsl-services/", {
            "customer": c1.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C01", "msag_port": 10,
        })
        resp = self.client.post("/api/dsl-services/", {
            "customer": c2.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C01", "msag_port": 10,
        })
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_same_port_number_on_different_card_is_allowed(self):
        c1 = self.make_customer()
        c2 = self.make_customer(name="Other", phone_primary="999", user_id="u999")
        r1 = self.client.post("/api/dsl-services/", {
            "customer": c1.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C01", "msag_port": 10,
        ...
        r2 = self.client.post("/api/dsl-services/", {
            "customer": c2.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C02", "msag_port": 10,
        })
        self.assertEqual(r1.status_code, status.HTTP_201_CREATED)
        self.assertEqual(r2.status_code, status.HTTP_201_CREATED)

    def test_port_63_is_valid_and_64_is_rejected(self):
        customer = self.make_customer()
        ok = self.client.post("/api/dsl-services/", {
            "customer": customer.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C09", "msag_port": 63,
        })
        self.assertEqual(ok.status_code, status.HTTP_201_CREATED, ok.data)

        c2 = self.make_customer(name="Other", phone_primary="999", user_id="u999")
        bad = self.client.post("/api/dsl-services/", {
            "customer": c2.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C10", "msag_port": 64,
        })
        self.assertEqual(bad.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("msag_port", bad.data)

    def test_negative_port_is_rejected(self):
        customer = self.make_customer()
        resp = self.client.post("/api/dsl-services/", {
            "customer": customer.id, "dsl_package": "X", "msag": self.msag.id, "msag_card": "C09", "msag_port": -1,
        })
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)


class CustomerOrderingTests(BaseAPITestCase):
    """The technician list can be sorted by MSAG cabinet and by port, not
    just name — for working through a cabinet/card in physical order."""

    def setUp(self):
        super().setUp()
        self.msag_b = MSAG.objects.create(exchange=self.exchange, code="AAA-FIRST-MSAG")

    def _dsl(self, customer, msag, card, port):
        return DSLService.objects.create(
            customer=customer, dsl_package="X", msag=msag, msag_card=card, msag_port=port,
        )

    def test_order_by_msag_code(self):
        c1 = self.make_customer(name="Z Customer")
        c2 = self.make_customer(name="A Customer", phone_primary="999", user_id="u999")
        self._dsl(c1, self.msag, "C01", 1)        # msag.code = "GOR-III LHR MSAG"
        self._dsl(c2, self.msag_b, "C01", 1)       # msag_b.code = "AAA-FIRST-MSAG"

        resp = self.client.get("/api/customers/?ordering=dsl_service__msag__code")
        names = [r["name"] for r in resp.data["results"]]
        self.assertEqual(names, ["A Customer", "Z Customer"])

    def test_order_by_port_number(self):
        c1 = self.make_customer(name="High Port")
        c2 = self.make_customer(name="Low Port", phone_primary="999", user_id="u999")
        self._dsl(c1, self.msag, "C01", 40)
        self._dsl(c2, self.msag, "C02", 2)

        resp = self.client.get("/api/customers/?ordering=dsl_service__msag_port")
        names = [r["name"] for r in resp.data["results"]]
        self.assertEqual(names, ["Low Port", "High Port"])

    def test_list_includes_msag_code_and_port(self):
        customer = self.make_customer()
        self._dsl(customer, self.msag, "C01", 7)
        resp = self.client.get("/api/customers/")
        row = resp.data["results"][0]
        self.assertEqual(row["msag_code"], self.msag.code)
        self.assertEqual(row["msag_port"], 7)


class MSAGReportingTests(BaseAPITestCase):
    """Dashboard's MSAG-centric breakdowns and card capacity utilization."""

    def test_dashboard_includes_customers_by_msag(self):
        customer = self.make_customer()
        DSLService.objects.create(
            customer=customer, dsl_package="X", msag=self.msag, msag_card="C01", msag_port=0,
        )
        resp = self.client.get("/api/dashboard/stats/")
        self.assertIn({"msag": self.msag.code, "count": 1}, resp.data["customers_by_msag"])

    def test_dashboard_includes_card_utilization(self):
        c1 = self.make_customer()
        c2 = self.make_customer(name="Other", phone_primary="999", user_id="u999")
        DSLService.objects.create(customer=c1, dsl_package="X", msag=self.msag, msag_card="C01", msag_port=0)
        DSLService.objects.create(customer=c2, dsl_package="X", msag=self.msag, msag_card="C01", msag_port=1)

        resp = self.client.get("/api/dashboard/stats/")
        card = next(c for c in resp.data["card_utilization"] if c["card"] == "C01")
        self.assertEqual(card["used_ports"], 2)
        self.assertEqual(card["capacity"], 64)
        self.assertAlmostEqual(card["percent_full"], round(2 / 64 * 100, 1))


class PeriodicReportTests(BaseAPITestCase):
    """The weekly / fortnightly (15-day) / monthly business report."""

    def test_default_period_is_weekly(self):
        resp = self.client.get("/api/activity/report/")
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.data["period"], "weekly")

    def test_invalid_period_rejected(self):
        resp = self.client.get("/api/activity/report/?period=yearly")
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_fortnightly_and_monthly_periods_work(self):
        for period in ("fortnightly", "monthly"):
            resp = self.client.get(f"/api/activity/report/?period={period}")
            self.assertEqual(resp.status_code, status.HTTP_200_OK)
            self.assertEqual(resp.data["period"], period)

    def test_report_reflects_activity_in_range(self):
        create_resp = self.client.post("/api/customers/", {
            "name": "WAPDA Wireless Office", "address_line": "WAPDA House, Lahore",
            "city": self.city.id, "phone_primary": "04299202783",
        })
        customer_id = create_resp.data["id"]
        self.client.post("/api/complaints/", {"customer": customer_id, "fault_description": "No dial tone"})

        resp = self.client.get("/api/activity/report/?period=weekly")
        self.assertGreaterEqual(resp.data["new_customers"], 1)
        self.assertGreaterEqual(resp.data["complaints_opened"], 1)
        self.assertEqual(resp.data["complaints_resolved"], 0)

    def test_report_excludes_events_outside_the_period(self):
        create_resp = self.client.post("/api/customers/", {
            "name": "Test Customer", "address_line": "x", "city": self.city.id, "phone_primary": "555",
        })
        old_log = ActivityLog.objects.get(
            entity_type=ActivityLog.EntityType.CUSTOMER, entity_id=create_resp.data["id"]
        )
        old_log.created_at = timezone.now() - timedelta(days=40)
        old_log.save(update_fields=["created_at"])

        resp = self.client.get("/api/activity/report/?period=monthly")
        self.assertEqual(resp.data["new_customers"], 0)
